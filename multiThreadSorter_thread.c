...
void sort(FILE *file, char* fileName){
   ...
   CSVrecord * frontRec = NULL;
	...
	// freeLL(frontRec);
	// free(frontRec);	
	//free(masterList);	//another tiff test; jk bad idea, this yields mem corruption
}
...
int main(int argc, char *argv[] ){ //-----------------------MAIN---------
	...
	if (argv[2][0]=='-'||argv[4][0]=='-'||argv[6][0]=='-'){
		fprintf(stderr,"cant have a flag without an argument!!\n");
		exit(EXIT_FAILURE);
	}
	if(hasCol==0){
...
//---------------------end testing sort function--------------------------------------
    ...
    if(pthread_mutex_init(&mutex2, NULL) != 0){
     ...
     if(pthread_mutex_init(&lockGlobals, NULL) != 0){
        printf("Error on lock5.\n");
    }
